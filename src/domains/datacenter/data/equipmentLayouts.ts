import type { DatacenterEquipmentLegacy } from '../types';

/**
 * 데이터센터 장비 배치 템플릿
 * 격자 기준으로 장비들을 2칸 간격으로 배치
 * @deprecated 레거시 함수 - 새로운 프로젝트에서는 Zustand 스토어 사용
 */
export const createDatacenterEquipmentLayout = (gridSize: number, cubeSize: number): DatacenterEquipmentLegacy[] => [
 ...
 */
export const createServerFocusedLayout = (gridSize: number, cubeSize: number): DatacenterEquipmentLegacy[] => [
  // 주로 서버로 구성
  { x: gridSize * 1, y: gridSize * 1, z: 0, width: cubeSize, depth: cubeSize, height: cubeSize, type: 'server' },
 ...
 * 장비 타입별 개수 계산
 */
export const getEquipmentTypeCount = (equipment: DatacenterEquipmentLegacy[]) => {
