...
export const mockDeleteResource = (id: number) => {
  console.warn(`MOCK: Deleting resource ${id}`);
  MOCK_DATA = MOCK_DATA.filter((r) => r.id !== id);
};

export const mockDeleteMultipleResources = (ids: number[]) => {
  console.warn(`MOCK: Deleting multiple resources ${ids.join(", ")}`);
...
export const mockUpdateMultipleResourceStatus = (
  ...
  MOCK_DATA = MOCK_DATA.map((r) => {
    if (ids.includes(r.id)) {
      return { ...r, status: status }; // 새 객체를 반환하여 불변성 유지
    }
    return r;
  });
};
...
// --- Datacenter 목 데이터 ---
export const MOCK_DATACENTERS: ServerRoom[] = [
  {
    id: 1,
    name: "서울 데이터센터",
    code: "DC-SEL-001",
...
export const MOCK_RACKS: Rack[] = [
  {
    ...
    department: "IT팀",
    managerId: 1,
    // datacenterId: 1, // MSW 필터링을 위한 편의용 필드
  },
//   {
//     id: 2,
//     rackName: "RACK-A02",
