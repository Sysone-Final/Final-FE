...
interface BabylonDatacenterStore {
  ...
  updateEquipmentPosition: (id: string, gridX: number, gridY: number, gridZ?: number) => void;
...
export const useBabylonDatacenterStore = create<BabylonDatacenterStore>((set, get) => ({
  // 초기 격자 설정
  ...
  updateEquipmentPosition: (id, gridX, gridY, gridZ = 0) => {
    ...
    // 유효성 검사 (자기 자신은 제외)
    if (!isValidPosition(gridX, gridY) || isPositionOccupied(gridX, gridY, id)) {
      return;
    }
